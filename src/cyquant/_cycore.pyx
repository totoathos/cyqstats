...
cdef class CyGroupedStreamStats:
    ...
    def add(self, group_ids, values):
        """
        Fast path: buffers (numpy/memoryview) int32/int64 + float64
        Fallback: iterable zip_longest without materializing lists.
        """
        cdef Py_ssize_t i, n, bad_i = -1
        cdef long bad_gid = 0

        # --- fast path int32
        try:
            cdef cnp.int32_t[:] gids32 = group_ids
            ...
            with nogil:
                for i in range(n):
                    cdef long gid = gids32[i]
                    if gid < 0 or gid >= self.n_groups:
                        bad_i = i
                        bad_gid = gid
                        break
                    agg_add(&self._aggs[gid], vals[i])

            if bad_i != -1:
                raise ValueError(f"group id {bad_gid} out of range [0, {self.n_groups})")
            return
        except TypeError:
            pass
        ...
        # --- fast path int64
        try:
            cdef cnp.int64_t[:] gids64 = group_ids
            cdef double[:] vals2 = values
            if gids64.shape[0] != vals2.shape[0]:
                raise ValueError("group_ids and values must have the same length")

            n = vals2.shape[0]
            with nogil:
                for i in range(n):
                    cdef long gid2 = <long>gids64[i]
                    if gid2 < 0 or gid2 >= self.n_groups:
                        bad_i = i
                        bad_gid = gid2
                        break
                    agg_add(&self._aggs[gid2], vals2[i])
        ...
        # --- fallback: streaming zip (no listas)
        sentinel = object()
        for g, v in zip_longest(group_ids, values, fillvalue=sentinel):
            ...
            agg_add(&self._aggs[idx], float(v))
    ...
    def merge(self, other):
        ...
        with nogil:
            for i in range(self.n_groups):
                agg_merge(&self._aggs[i], &o._aggs[i])
    ...
    def result_arrays(self):
        ...
        sums = [0.0] * self.n_groups
        means = [nan] * self.n_groups
        mins = [nan] * self.n_groups
        maxs = [nan] * self.n_groups
        vars_ = [nan] * self.n_groups
        stds = [nan] * self.n_groups
